...
library("Seurat")
library("Signac")
load("integrated-celltype.Rdata")
DefaultAssay(integrated) <- 'RNA'

##seurat对象转换成scanpy anndata对象
# 矩阵信息
write.table(t(as.matrix(integrated@assays$RNA@counts)),"integrated_counts.csv",
            sep="\t",quote=FALSE,row.names=T,col.names=T)
# 降维信息
embedding <- Embeddings(integrated, "umap")
write.table(embedding,"integrated_embedding.csv",
            sep="\t",quote=FALSE,row.names=T,col.names=T)
# metadata
write.table(integrated@meta.data,"integrated_metadata.csv",
            sep="\t",quote=FALSE,row.names=T,col.names=T)

##seurat空转数据转成scanpy格式
#两种方式后续都报错,不采用
write.table(t(as.matrix(heart_merge@assays$Spatial@counts)),"heart_merge_counts.csv",
            sep="\t",quote=FALSE,row.names=T,col.names=T)
# 降维信息
embedding <- Embeddings(heart_merge, "umap")
write.table(embedding,"heart_merge_embedding.csv",
            sep="\t",quote=FALSE,row.names=T,col.names=T)
# metadata
write.table(heart_merge@meta.data,"heart_merge_metadata.csv",
            sep="\t",quote=FALSE,row.names=T,col.names=T)

library(SeuratDisk)
SaveH5Seurat(heart_merge, filename = "heart_merge.h5Seurat")
Convert("heart_merge.h5Seurat", dest = "h5ad")

##2.cell2location的单细胞和空转需要raw count数据作为input！
...
##scanpy空转数据分析
Control1 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/B468/outs',library_id="Control-1")
Control2 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/B471/outs',library_id="Control-2")
ISO1d1 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/A936/outs',library_id="ISO1d-1")
ISO1d2 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/B479/outs',library_id="ISO1d-2")
ISO3d1 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/A945/outs',library_id="ISO3d-1")
ISO3d2 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/B493/outs',library_id="ISO3d-2")
ISO7d1 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/A935/outs',library_id="ISO7d-1")
ISO7d2 = sc.read_visium('/mnt/sda/jwz/data/TTS/spatial/spaceranger/A943/outs',library_id="ISO7d-2")

Control1.var_names_make_unique()
Control2.var_names_make_unique()
ISO1d1.var_names_make_unique()
ISO1d2.var_names_make_unique()
ISO3d1.var_names_make_unique()
ISO3d2.var_names_make_unique()
ISO7d1.var_names_make_unique()
ISO7d2.var_names_make_unique()

#生成质控指标
sc.pp.calculate_qc_metrics(Control1, inplace=True)
sc.pp.calculate_qc_metrics(Control2, inplace=True)
sc.pp.calculate_qc_metrics(ISO1d1, inplace=True)
sc.pp.calculate_qc_metrics(ISO1d2, inplace=True)
sc.pp.calculate_qc_metrics(ISO3d1, inplace=True)
sc.pp.calculate_qc_metrics(ISO3d2, inplace=True)
sc.pp.calculate_qc_metrics(ISO7d1, inplace=True)
sc.pp.calculate_qc_metrics(ISO7d2, inplace=True)

##合并原始矩阵andata对象
adata_spatial = Control1.concatenate(Control2,ISO1d1,ISO1d2,ISO3d1,ISO3d2,ISO7d1,ISO7d2,batch_key="library_id",uns_merge="unique",batch_categories=["Control-1","Control-2","ISO1d-1","ISO1d-2","ISO3d-1","ISO3d-2","ISO7d-1","ISO7d-2"])
adata_spatial.obs['sample']=adata_spatial.obs['library_id']
adata.write("/mnt/sda/jwz/data/TTS/spatial/spaceranger/scanpy/adata_spatial_original.h5ad")
